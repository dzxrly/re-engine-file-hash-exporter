...
- 提供 `auto_detect` 和 `profile_then_range` 候选规划模式，可从根目录 `file_suffix_profiles.json` 读取可编辑预设。
...
| `mode` | 字符串，默认 `"small_range"` | 候选版本规划模式。可选值：`"small_range"`、`"adaptive"`、`"custom"`、`"auto_detect"`、`"profile_then_range"`。 |
| `min_version` | 整数，默认 `0` | 在 `small_range` 中表示起始版本号；在 `auto_detect` 的 numeric profile 中表示从预设最小值向下扩展多少。`profile_then_range` 会忽略此字段。字段生效时必须非负。 |
| `max_version` | 整数，默认 `4096` | 在 `small_range` 中表示结束版本号；在 `auto_detect` 的 numeric profile 中表示从预设最大值向上扩展多少；在 `profile_then_range` 的 numeric profile 中表示最新预设版本之后新增区间的结束值。 |
| `custom_versions` | 字符串，默认空 | 仅用于 `custom` 模式。支持逗号、换行和范围，例如 `"12,18,30-40"`。 |
| `neighbor_radius` | 整数，默认 `32` | 仅用于 `adaptive` 模式，对已知版本号左右各扩展这个半径。 |
| `date_start` | 字符串，默认空 | 用于 `auto_detect` 的 `date_code` profile，表示向最早优先日期之前扩展多少天。`profile_then_range` 会忽略此字段。 |
| `date_end` | 字符串，默认空 | 用于 profile 引导的 `date_code` 搜索，表示向最晚优先日期之后扩展多少天。设置为 `"today"` 时会扩展到本机当前日期且不会缩小预设范围；在 `profile_then_range` 中它是日期类型自己的上界。 |
...
由于 DMP 中可能包含已被覆盖或不完整的内存字符串，Step 1 只接受结束于可信 UTF-16 文本边界的路径。数字后缀后面直接连接二进制数据时，该残片会被忽略并输出警告，不会写入 `suffix_map`。
...
- `profile_then_range`：先搜索 profile 中已有的候选，再只搜索最新已知值之后的新增区间。`numeric` 类型先尝试全部 `priority_versions`，然后搜索 `max(priority_versions) + 1 .. Max version`，忽略 `Min version`；没有 numeric 预设时回退到 `0..Max version`。`date_code` 类型先尝试全部 `priority_dates × priority_tails` 组合，再从最新组合之后按有效日历后缀搜索到 `Date +days`（或 `today`），忽略 `Date -days` 和 numeric 的 `Max version`。numeric 与 date-code 扩展名可以混选，因为两者分别使用自己的上界。

一般建议：需要复查 profile 已知版本并只继续搜索较新值时使用 `profile_then_range`；需要更宽的 profile 引导范围时使用 `auto_detect`；Step 1 已找到相关版本时可试 `adaptive`；需要完整范围时用 `small_range`；已知候选时用 `custom`。
...
如果某个 PAK group 以 patch 文件开始，且没有加载对应 base PAK，Step 2 仍会使用增量模式。通常初始下界来自所选扩展名在 `file_suffix_profiles.json` 中的基准版本，后续同组 patch 会继续从前面 patch 已发现的版本之后搜索。`profile_then_range` 会先在第一个 patch-only group 中搜索 profile 已知候选，之后的 patch 再应用增量下界。
